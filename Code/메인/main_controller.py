...
class MainController():
    def __init__(self, Voice, MainView):
    ...
    def playVoice(self):
        ...
        if paused:
            mixer.music.unpause()
            paused = False
            t1 = threading.Thread(target=self.__startCount, args=(total_length,))
            t1.start()
        else:
            mixer.music.stop()
            mixer.music.play()
    ...
    def __startCount(self, t):
        ...
        while current_time <= t and mixer.music.get_busy():
            if paused:
                ...
                self.__view.updateSoundPgrBar((mixer.music.get_pos() / 1000) / t * 100)
                time.sleep(1)
                current_time += 1
        self.__view.updateSoundPgrBar(100)
